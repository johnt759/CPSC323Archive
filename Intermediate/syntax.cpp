...
bool Analyzer::isRelation(int& tokenIndex)
{
	if (tokenIndex > numTokens)
		return false;

	// Be sure to look through all 6 of the important relational operators!
	if (currentToken[tokenIndex] == "<")
	{
		cout << "Token: " << tokenType[tokenIndex] << "\tLexeme: " << currentToken[tokenIndex] << "\n";
		tokenIndex++;
...
bool Analyzer::isCondition(int& tokenIndex)
{
	if (tokenIndex > numTokens)
		return false;
	...
	// When verifying the conditional statement, be sure to inspect the relational operator as well.
	if (isIf(tokenIndex))
	{
		if (isLeftParentheses(tokenIndex))
		{
			if (isIdentifier(tokenIndex))
			{
				if (isRelation(tokenIndex))
				{
					if (isIdentifier(tokenIndex))
					{
						if (isRightParentheses(tokenIndex))
						{
						cout << "<Condition>-><If>(<Identifier><RelationOperator><Identifier>)\n";
						return true;
						}
					}
				}
	...
	else if (isElse(tokenIndex))
	{
		cout << "<Condition>-><Else>\n";
		return true;
	}
	else if (isWhile(tokenIndex))
	{
		if (isLeftParentheses(tokenIndex))
		{
			if (isIdentifier(tokenIndex))
			{
				if (isRelation(tokenIndex))
				{
					if (isIdentifier(tokenIndex))
					{
						if (isRightParentheses(tokenIndex))
						{
							cout << "<Condition>-><While>(<Identifier><RelationOperator><Identifier>)\n";
							return true;
						}
					}
				}
...
bool Analyzer::isIf(int& tokenIndex)
{
	if (tokenIndex > numTokens)
		return false;

	if (currentToken[tokenIndex] == "if")
	{
		cout << "Token: " << tokenType[tokenIndex] << "\tLexeme: " << currentToken[tokenIndex] << "\n";
		tokenIndex++;
		return true;
...
bool Analyzer::isElse(int& tokenIndex)
{
	if (tokenIndex > numTokens)
		return false;

	if (currentToken[tokenIndex] == "else")
	{
		cout << "Token: " << tokenType[tokenIndex] << "\tLexeme: " << currentToken[tokenIndex] << "\n";
		tokenIndex++;
		return true;
...
bool Analyzer::isWhile(int& tokenIndex)
{
	if (tokenIndex > numTokens)
		return false;

	if (currentToken[tokenIndex] == "while")
	{
		cout << "Token: " << tokenType[tokenIndex] << "\tLexeme: " << currentToken[tokenIndex] << "\n";
		tokenIndex++;
		return true;
